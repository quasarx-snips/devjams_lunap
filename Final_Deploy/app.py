"""
LunaP backend server
=====================
Wraps the two modules the frontend needs:

  1. CLAHE enhancement  -> baseline image used for every downstream step
  2. Hazard-map engine  -> main.generate_hazard_overlay() (AI U-Net + morphology)
...
def _ensure_hazard_ready(image_data: str = None):
    ...
    if image_data:
        img = data_url_to_cv2(image_data)
        if LAST_RESULT.get("image_hash") != _image_hash(img):
...
def hazard_map():
    ...
    try:
        ...
        img = data_url_to_cv2(image_data)
        out_path = _run_hazard_pipeline(img)
