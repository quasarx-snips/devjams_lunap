...
def generate_hazard_overlay(image_path: str, model_path: str, output_dir: str = "Outlay", show: bool = True) -> str:
    ...
    # Persist the raw numeric risk map alongside the overlay PNG. Landing-site
    # scoring, route planning, and 3D terrain all need this exact array --
    # saving it here means they reuse the hazard map already computed instead
    # of re-running the (expensive) AI + morphology pipeline.
    risk_path = os.path.splitext(out_path)[0] + "_risk.npy"
    np.save(risk_path, final_risk_map)
